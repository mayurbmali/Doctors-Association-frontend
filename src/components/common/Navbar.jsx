...
const Navbar = () => {
  ...
  return (
    <nav className={`sticky top-0 z-50 transition-all duration-400 ${
      isScrolled 
        ? 'bg-white/98 backdrop-blur-lg shadow-lg border-b border-neutral-200' 
        : 'bg-white shadow-md'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <Link 
            to="/" 
            className="flex items-center space-x-3 group transition-all duration-400 hover:scale-105 flex-shrink-0"
          >
            <img 
              src="/ccmp-logo.png" 
              alt="CCMP Homoeopathic Doctors Association Logo" 
              className="h-14 w-14 object-contain group-hover:scale-110 group-hover:rotate-3 transition-transform duration-400"
            />
            <div>
              <h1 className="text-lg sm:text-xl font-bold bg-gradient-to-r from-orange-500 to-orange-600 bg-clip-text text-transparent leading-tight">
                CCMP
              </h1>
                ...
                <Link
                  ...
                  className={`relative px-3 py-2 text-sm font-semibold transition-all duration-300 rounded-lg group ${
                    isActive(item.href)
                      ...
                      : "text-neutral-700 hover:text-orange-600 hover:bg-orange-50/50"
                  }`}
                >
                  {item.name}
                  <span className={`absolute bottom-1 left-1/2 transform -translate-x-1/2 h-0.5 bg-gradient-to-r from-orange-500 to-orange-600 transition-all duration-300 ${
                    isActive(item.href) 
                      ? "w-6 opacity-100" 
                      : "w-0 opacity-0 group-hover:w-4 group-hover:opacity-100"
                  }`}></span>
          ...
          <div className="hidden lg:flex items-center space-x-3 flex-shrink-0">
            ...
            <Button 
              asChild 
              className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-6 py-2.5 rounded-lg shadow-md hover:shadow-lg hover:shadow-orange-500/30 transition-all duration-400 transform hover:scale-105"
            >
              <Link to="/membership" className="flex items-center space-x-2">
                <span>Join CCMP</span>
                <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
            ...
            <div className="relative" ref={langDropdownRef}>
              <button
                onClick={() => setLangDropdownOpen(!langDropdownOpen)}
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 border-neutral-200 hover:border-orange-500 hover:bg-orange-50 transition-all duration-300 group"
              >
                <Globe className="w-4 h-4 text-neutral-600 group-hover:text-orange-600 transition-colors" />
                <span className="text-sm font-medium text-neutral-700 group-hover:text-orange-600 transition-colors">
                  {languages.find(l => l.code === currentLang)?.flag}
                </span>
                <ChevronDown className={`w-4 h-4 text-neutral-500 transition-transform duration-300 ${langDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              
              {langDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-2xl border border-neutral-200 overflow-hidden z-50 animate-fade-in">
                  <div className="py-1">
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
                        onClick={() => handleLanguageChange(lang.code)}
                        className={`w-full flex items-center justify-between px-4 py-3 hover:bg-orange-50 transition-all duration-200 ${
                          currentLang === lang.code ? 'bg-orange-50' : ''
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <span className="text-xl">{lang.flag}</span>
                          <span className={`text-sm font-medium ${
                            currentLang === lang.code ? 'text-orange-600' : 'text-neutral-700'
                          }`}>
                            {lang.name}
                          </span>
                        </div>
          ...
          <div className="lg:hidden">
            <button
              onClick={() => setIsOpen(!isOpen)}
              className="relative p-3 text-neutral-700 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all duration-300"
            >
              <div className="relative w-6 h-6">
                <Menu className={`w-6 h-6 absolute transition-all duration-300 ${isOpen ? 'rotate-90 opacity-0' : 'rotate-0 opacity-100'}`} />
                <X className={`w-6 h-6 absolute transition-all duration-300 ${isOpen ? 'rotate-0 opacity-100' : '-rotate-90 opacity-0'}`} />
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-500 ease-in-out ${
          ...
          <div className="px-2 pt-2 pb-6 space-y-2 bg-neutral-50 rounded-2xl mx-2 mb-4 shadow-xl border border-neutral-200">
            {navigation.map((item, index) => (
              <Link
                key={item.name}
                to={item.href}
            ...
            <div className="px-2 pt-2 border-t border-neutral-300 mt-2">
              <p className="text-xs font-semibold text-neutral-500 uppercase px-2 mb-2">Language</p>
              <div className="space-y-1">
                {languages.map((lang) => (
                  <button
                    key={lang.code}
            ...
            <div className="px-2 pt-4">
              <Button 
                asChild 
                className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-4 rounded-xl shadow-lg hover:shadow-xl hover:shadow-orange-500/30 transition-all duration-400"
              >
                <Link 
                  to="/membership" 
                  onClick={() => setIsOpen(false)}
