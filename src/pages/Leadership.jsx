          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            {committees.map((committee) => (
              <TabsContent key={committee.id} value={committee.id}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {committee.data.map((member, index) => (
                    <Card key={index} className="hover:shadow-xl hover:shadow-orange-500/30 transition-all duration-300 transform hover:-translate-y-2">
                      <CardContent className="p-6">
                        {/* Avatar */}
                        <div className="text-center mb-4">
                          <div className="w-20 h-20 bg-gradient-to-br from-orange-500 to-green-500 rounded-full flex items-center justify-center text-white text-2xl font-bold mx-auto mb-3">
                            {member.name.charAt(0)}
                          </div>
                          <Badge 
                            className={`text-xs ${
                              committee.id === "advisory" ? "bg-orange-100 text-orange-800" :
                              committee.id === "executive" ? "bg-orange-100 text-orange-800" :
                              committee.id === "steering" ? "bg-green-100 text-green-800" :
                        ...
                        <div className="text-center">
                          <h3 className="text-lg font-bold text-gray-900 mb-2">{member.name}</h3>
                          <p className="text-green-500 font-semibold mb-2">{member.designation}</p>
                          <p className="text-gray-600 text-sm mb-4">{member.specialization}</p>
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              ...
              <Card key={index} className="hover:shadow-xl hover:shadow-orange-500/30 transition-all duration-300 transform hover:-translate-y-2">
                <CardContent className="p-8">
                  <div className="flex items-center mb-6">
                    <div className={`inline-flex items-center justify-center w-12 h-12 ${committee.color === 'orange' ? 'bg-orange-100' : 'bg-green-100'} rounded-lg mr-4`}>
                      {committee.icon}
                    </div>
                    <h3 className="text-xl font-bold text-gray-900">{committee.title}</h3>
                  </div>
                  <ul className="space-y-3">
                    {committee.responsibilities.map((responsibility, i) => (
                      <li key={i} className="flex items-start">
                        <div className={`w-2 h-2 ${committee.color === 'orange' ? 'bg-orange-500' : 'bg-green-500'} rounded-full mt-2 mr-3 flex-shrink-0`}></div>
                        <span className="text-gray-700">{responsibility}</span>
                      </li>
                    ))}
                  </ul>
